...
if __name__ == '__main__':
    ...
    if (study_error):
        ...
        k = T/J_cte
        N_space_st = np.arange(10,2000,10) #20
        error_list = []
        ...
        plt.figure()
        plt.plot(np.log10(1/N_space_st),np.log10(error_list))
        plt.xlabel("log(h)")
        ...
        lr_h = st.linregress(np.log10(1/N_space_st),np.log10(error_list))
        print("Linear regression for h: ",lr_h)
        
        
        #Compute error for a fixed space step N=100
        N_cte = 10
        error_list_2 = []
        J_time_st = np.arange(500,2000,5)
        for J_ in J_time_st:
        ...
        #Plot the error in log scale
        plt.figure()
        plt.plot(np.log10(T/J_time_st),np.log10(error_list_2))
        plt.xlabel("log(k)")
        plt.ylabel("log(error)")
        ...
        lr_k = st.linregress(np.log10(T/J_time_st),np.log10(error_list_2))
        print("Linear regression for k: ",lr_k)
